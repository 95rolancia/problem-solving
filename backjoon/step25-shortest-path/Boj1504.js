/*
    문제 : 특정한 최단 경로 (https://www.acmicpc.net/problem/1504)
    난이도 : 골드 4
*/
const fs = require('fs');
const stdin = (
...
const [stopover1, stopover2] = input().split(' ').map(Number);

const d1 = Array(V + 1).fill(Number.MAX_SAFE_INTEGER);
dijkstra(1, d1);

const d2 = Array(V + 1).fill(Number.MAX_SAFE_INTEGER);
dijkstra(stopover1, d2);

const d3 = Array(V + 1).fill(Number.MAX_SAFE_INTEGER);
dijkstra(stopover2, d3);

const answer = Math.min(
  d1[stopover1] + d2[stopover2] + d3[V],
  d1[stopover2] + d3[stopover1] + d2[V]
);
